...
export class ModulosPage implements OnInit {
  ...
  public columns = [
    // { prop: 'codigo', name: 'Código', width: 20 },
    { prop: 'modulo', name: 'Módulo' },
    { prop: 'nome', name: 'Nome' }
  ]
  ...
  ionViewDidEnter() {
    this.CRUDModulos('READ','');
  }

  private CRUDModulos(_statusCRUD, _formValues) {
    this.sendRequest('spModulos', {
      ...
      this.modulos = results.map(function callback(value) {
        let modulo = new Modulo();
        modulo.codigo = value.CODIGO;
        modulo.nome = value.nome;
        ...
        this.Authorizer.QueryStoreProc('Executar', procedure, _params).then(res => {
          ...
          try {
            if (resultado.success) {
              ...
              this.alertService.presentAlert({ pTitle: 'ATENÇÃO', pSubtitle: ModulosPage.APP_NAME, pMessage: resultado.message });
            }
          } catch (err) {
            this.alertService.presentAlert({ pTitle: this.env.AppNameSigla, pSubtitle: ModulosPage.APP_NAME, pMessage: 'Erro ao fazer a petição' });
          }
        });
      } else {
        this.alertService.presentAlert({
          pTitle: 'SEM PERMISSÃO', pSubtitle: ModulosPage.APP_NAME, pMessage: 'Você não tem permissão para esta ação'
        });
      }
    } else {
      this.goBack();
