...
export class CartaoPage implements OnInit {
  ...
  ionViewDidEnter() {
    this.CRUDCartaos(CartaoPage.CRUD_READ, '');
  }
  ...
  private CRUDCartaos(_statusCRUD, _formValues) {
    this.sendRequest('spCartaos', {
      ...
      this.cartaos = results.map(function callback(value) {
        let cartao = new Cartao();
        cartao.codigo = value.codigo;
        cartao.cartao = value.cartao;
        cartao.cerca = value.cerca;
        return cartao;
      });
      ...
      buttons: [
        {
          ...
          text: 'Sim',
          handler: () => {
            this.CRUDCartaos(CartaoPage.CRUD_DELETE, { 'codigo': id });
          }
        }
      ]
    });
        ...
        this.Authorizer.QueryStoreProc('Executar', procedure, _params).then(res => {
          ...
          try {
            if (resultado.success) {
              ...
              this.alertService.presentAlert({ pTitle: 'ATENÇÃO', pSubtitle: CartaoPage.APP_NAME, pMessage: resultado.message });
            }
          } catch (err) {
            this.alertService.presentAlert({ pTitle: this.env.AppNameSigla, pSubtitle: CartaoPage.APP_NAME, pMessage: 'Erro ao fazer a petição' });
          }
        });
      } else {
        this.alertService.presentAlert({
          pTitle: 'SEM PERMISSÃO', pSubtitle: CartaoPage.APP_NAME, pMessage: 'Você não tem permissão para esta ação'
        });
      }
    } else {
      this.goBack();
